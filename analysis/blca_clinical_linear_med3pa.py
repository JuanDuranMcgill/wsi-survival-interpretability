#!/usr/bin/env python3
# blca_clinical_linear_med3pa.py  — revised for AIM revision
#
...
#      Old med3pa_error_input.npz was in-sample and censoring-defective.
#  2b. Locked discovery/evaluation split: profiles discovered on 60%,
#      all reported numbers evaluated on the other 40%.
#  2c. Metrics: AUROC + balanced accuracy on eval set (not t-test vs 0.50);
#      per-profile survival c-index with bootstrap CI on eval set.
...
#      thresholds reported in both z-scored and clinical-unit form.
#  2f. BLCA consistency fix: default SAVE_ROOT now points at the 102-round
#      med3pa_bootstrap_intermediate run, not the 25-round med3pa_bootstrap.
...
# -------------------------------------------------------
# Narval paths (2a + 2f: updated from Trillium defaults)
# All four data items must be present in ~/data/ before this runs.
# -------------------------------------------------------
_DATA = os.path.expanduser("~/data")
CLINICAL_CSV = os.path.join(_DATA, "clinical_features.csv")
# 2f: 102-round run (was med3pa_bootstrap for 25-round; see RUNBOOK §2f)
SAVE_ROOT    = os.path.join(_DATA, "med3pa_bootstrap_intermediate")
# 2a: OOB error file produced by analysis/oob_risk.py (censoring-fixed)
...
# -------------------------------------------------------
# Step 1 – load and align backbone + clinical + OOB error
# -------------------------------------------------------
def load_and_align_data(clinical_csv, error_npz, save_root):
    """Return aligned arrays plus survival data from the OOB npz.

    The OOB npz (patient_error_blca_oob.npz) has keys:
        patient_ids, error, n_comparable_pairs, risk, pfi_time, pfi_event
    It replaces both the old final_med3pa_input.npz (for risk scores)
    and the old med3pa_error_input.npz (for error labels). (2a)
    """
...
def apply_profile_to_X(conditions_median, X_eval, feature_names):
    """Apply a profile's median-threshold rules to X_eval, return boolean mask.

    `conditions_median` is a dict {cond_key: median_threshold} where cond_key
    is like "T_stage_DOWN" (meaning T_stage <= threshold) or "age_UP" (age > threshold).
    """
...
def run_single_med3pa(run_idx, X_disc, y_disc, feature_names, X_eval,
                       ...
                       outdir, save_runs):
    """Discover profiles on X_disc, return (profiles, eval_scores).

    2b: only discovery set is passed in; caller holds eval set separate.
    The secondary RF is fitted on x_train (subset of disc) and its
    predict_proba on X_eval is returned so the caller can compute
    classifier_metrics on honest held-out data.
    """
    seed = 1000 + run_idx
    ...
    # Eval-set scores from the secondary classifier (2c)
...
def aggregate_profiles(all_run_profiles, n_runs_requested):
    ...
    n_completed = 0
    for run_profiles, _ in all_run_profiles:   # each entry is (profiles, eval_scores)
    ...
    for key, records in profile_records.items():
        ...
        rows.append({
            ...
            # Needed by apply_profile_to_X for profile_report (2c)
...
# -------------------------------------------------------
# 2e: StandardScaler inverse-transform for thresholds
# -------------------------------------------------------
def make_scaler_params(X, feature_names):
    """Fit StandardScaler on X; return per-feature (mean, scale) for threshold inversion.

    If the input CSV is pre-z-scored (mean≈0, scale≈1) the inverse transform
    is a no-op and the clinical_value column will equal the z-scored threshold.
    If it contains raw clinical values the inversion recovers clinical units.
    Either way, both columns are reported so the reader can judge.
    """
    sc = StandardScaler()
    sc.fit(X)
...
def main():
    ap = argparse.ArgumentParser(
        description="Clinical-only pipeline: RF importance + MED3PA reliability"
    )
    ap.add_argument("--clinical-csv", type=str, default=CLINICAL_CSV)
    ap.add_argument("--error-npz", type=str, default=ERROR_NPZ,
                    help="OOB error npz from analysis/patient_error.py (keys: patient_ids, error, risk, pfi_time, pfi_event)")
    ap.add_argument("--save-root", type=str, default=SAVE_ROOT,
                    help="Backbone run directory; used only for metadata (round count inference).")
    ap.add_argument("--round-count", type=int, default=102,
                    help="Number of bootstrap rounds the backbone ran (BLCA default: 102).")
    ap.add_argument("--outdir", type=str, default="results/reliability_blca_clinical")
    ...
    ap.add_argument("--med3pa-error-quantile", type=float, default=0.5,
                    help="Quantile threshold for high-error label. Primary: 0.5 (median split). Sensitivity: 0.7, 0.8, 0.9.")
    ap.add_argument("--discovery-frac", type=float, default=0.6,
                    help="Fraction of patients used for MED3PA profile discovery. Remainder is eval.")
    ap.add_argument("--split-seed", type=int, default=20260908,
                    help="Seed for the locked discovery/eval split. Fixed across all runs of this cohort.")
    ...
    print(f"[blca_clinical] {len(patient_ids)} patients aligned")

    # ---- Step 2a: feature importance vs survival_target ----
    ...
    # ---- 2e: StandardScaler for threshold inverse-transform ----
    scaler_params = make_scaler_params(X, feature_names)

    # ---- MED3PA label from OOB backbone error (2d quantile) ----
    ...
    print(f"[blca_clinical] q{args.med3pa_error_quantile:.2f} threshold={thresh:.4f}, "
          f"prevalence={prevalence_overall:.3f}, majority_acc={max(prevalence_overall, 1-prevalence_overall):.3f}")

    # ---- 2b: Locked discovery/eval split ----
    ...
    risks_eval = y[eval_mask]   # y holds WSI risk scores (from oob npz "risk")
    print(f"[blca_clinical] discovery N={disc_mask.sum()}, eval N={eval_mask.sum()}")

    # ---- MED3PA profile discovery on discovery cohort only ----
    med3pa_params = {
    ...
    print(f"[blca_clinical] eval AUROC={cm.get('auroc', 'N/A'):.3f}, "
    ...
    # ---- 2c + 2e: per-profile eval metrics with inverse-transformed thresholds ----
    profile_reports = []
    if not agg_df.empty:
        ...
        for _, row in top_profiles.iterrows():
            ...
            # 2e: inverse-transform thresholds
            readable_clinical = {}
    ...
    # ---- JSON output with all required metadata (results/ contract) ----
    out = {
        "cohort": "blca",
        "arm": "clinical",
        "endpoint": "PFI",
        ...
        "scaler_params_note": (
            ...
            "(scale≈1, mean≈0). Use the original preprocessing scaler for exact "
            "clinical-unit thresholds."
        ),
    ...
    print(f"[blca_clinical] wrote {out_json}")
    print(f"[blca_clinical] saved outputs to {args.outdir}")
