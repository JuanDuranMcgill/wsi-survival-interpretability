#!/usr/bin/env python3
# brca_radiomic_linear_med3pa.py
#
# BRCA radiomics-only pipeline:
#   Step 1: load backbone outputs + BRCA radiomics CSV, align patients.
#   Step 2: RF importance vs survival_target (WSI risk score), consistent with clinical pipeline:
#           - restrict to FIRST-ORDER radiomic features only
#           - bootstrap RandomForestRegressor to get stable feature ranks
#           - save radiomic importance + summaries + comparison table
#   Step 3: MED3PA on backbone error:
#           - define high-error label from backbone_error quantile
#           - run MED3PA using a selected top-K subset of first-order
#             radiomic features as inputs
#           - aggregate direction-based profiles and save results.
#
# Notes:
#   - MED3PA label is ALWAYS derived from backbone_error (not radiomic model error).
#   - Only FIRST-ORDER radiomic features are used, and MED3PA sees only top-K.
...
# -------------------------------------------------------
# Paths (BRCA radiomics + backbone)
# -------------------------------------------------------
RADIOMICS_CSV = "/home/sorkwos/links/scratch/radiomics_output_BRCA/radiomics_pre_corr.csv"
...
# -------------------------------------------------------
# Feature name parsing — BRCA region prefixes
# -------------------------------------------------------
KNOWN_REGIONS = [
    "necrosis", "dcis", "til", "tils", "immune",
    "invasive", "tumor", "muscle", "tdlu", "normal",
    "adipose", "fat", "vessels", "blood_vessels",
    "stroma", "fibrous",
]
...
def parse_feature_components(feat_name):
    """
    Parse a BRCA radiomic feature name of the form:
...
# -------------------------------------------------------
# Step 1 – load and align backbone + BRCA radiomics
# -------------------------------------------------------
def load_and_align_data(radiomics_csv=RADIOMICS_CSV):
    """
    Load BRCA radiomics, backbone survival target (mean_risk),
...
def run_radiomic_importance_firstorder(
    ...
    """
    Harrell-style radiomic importance for BRCA:
      ...
      - return indices / names of top-N first-order features for MED3PA.
    """
    ...
    print(">>> BRCA radiomics (first-order-only) for Harrell-style importance")
    ...
    # Region / class / channel / stat summaries
    region_rows = []
    ...
    print(f"\n>>> Saved BRCA radiomic importance outputs to {outdir}")
    for p in [feat_path, region_path, class_path, channel_path, stat_path, comp_path]:
        print(f"    {p}")
    ...
    print(f"\n>>> Selected top {k} first-order radiomic features for BRCA MED3PA.")
    return X_fo, survival_target, fo_feature_names, top_indices_for_med3pa, selected_feature_names

# -------------------------------------------------------
# Step 3 – MED3PA on backbone error with BRCA radiomic features
# -------------------------------------------------------
...
def run_single_med3pa(
    ...
    """Run one BRCA Med3PA experiment and return extracted profiles."""
    seed = 1000 + run_idx
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "BRCA radiomics-only pipeline: "
            "first-order radiomic error analysis + MED3PA on backbone error"
        )
    )

    # Radiomics / importance
    ap.add_argument("--radiomics-csv", type=str, default=RADIOMICS_CSV)
    ap.add_argument("--outdir", type=str, default="results/brca_radiomic_linear_med3pa")
    ...
    # Step 1 – load BRCA radiomics + backbone
    print(">>> Loading and aligning BRCA radiomics + backbone...")
    ...
    # Slice matrix down to selected top-K first-order features for MED3PA
    X_med3pa = X_fo[:, top_idx_med3pa]
    print(f"\n>>> MED3PA will use {X_med3pa.shape[1]} first-order BRCA radiomic features as inputs.")

    # Step 3 – MED3PA label from backbone_error (NOT radiomic model error)
    ...
    print(f"    Saved BRCA radiomic MED3PA label table: {label_path}")
    ...
    print(
        f"\n>>> Running {args.n_runs} BRCA Med3PA bootstrap iterations "
    ...
    print("\n>>> Aggregating BRCA radiomic MED3PA profiles (direction-based matching)...")
    ...
    if agg_df.empty:
        ...
        print(f"    Saved all BRCA radiomic MED3PA profiles: {all_path}")
        ...
        for min_freq in [10, 25, 50]:
            ...
            print(f"    Saved BRCA radiomic profiles appearing in >= {min_freq} runs: {fpath}")

        print(f"\n>>> BRCA radiomic MED3PA results summary:")
        ...
        print(f"\n>>> Top 20 BRCA radiomic profiles by bootstrap frequency:")
    ...
    with open(os.path.join(args.outdir, "README_outputs_radiomic.txt"), "w") as f:
        f.write("BRCA radiomics-only pipeline outputs (first-order only)\n")
        ...
        f.write("- radiomic_comparison_table.csv: top radiomic features vs continuous backbone_error\n")
        f.write("- med3pa_backbone_error_labels_radiomic.csv: high/low-error label from backbone_error\n")
        f.write("- all_profiles_dir_radiomic.csv, profiles_freq*_radiomic.csv: aggregated BRCA radiomic MED3PA profiles\n")
        f.write("- med3pa_run_summary_radiomic.csv: BRCA MED3PA run counts + error quantile\n")

    print(f"\n>>> Done. Saved BRCA radiomic outputs to {args.outdir}")
