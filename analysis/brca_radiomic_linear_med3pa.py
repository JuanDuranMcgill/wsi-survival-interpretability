#!/usr/bin/env python3
# brca_radiomic_linear_med3pa.py  — revised for AIM revision
#
...
def run_radiomic_importance_firstorder(X, survival_target, feature_names, n_trees,
    ...
    print(f">>> BRCA radiomics (first-order-only): {len(fo_feature_names)} / {len(feature_names)} features")
    ...
    print(f"    Saved BRCA radiomic importance outputs to {outdir}")
    ...
    print(f"    Selected top {k} first-order features for BRCA MED3PA.")
    fi_metrics = {
...
def main():
    ap = argparse.ArgumentParser(
        description="BRCA radiomic pipeline: first-order importance + MED3PA reliability"
    )
    ...
    ap.add_argument("--outdir", type=str, default="results/reliability_brca_radiomic")
    ...
    print(">>> Loading BRCA radiomics + OOB backbone...")
    ...
    print(f">>> Running {args.n_runs} BRCA MED3PA iterations ({args.n_parallel} workers)...")
    ...
    if not agg_df.empty:
        ...
        print(f"\n>>> Top BRCA radiomic profiles by bootstrap frequency:")
    ...
    out_json = os.path.join(args.outdir, "reliability_brca_radiomic.json")
    with open(out_json, "w") as f:
        json.dump(out, f, indent=2)
    print(f">>> Wrote {out_json}")
    print(f">>> Done. BRCA radiomic outputs in {args.outdir}")
